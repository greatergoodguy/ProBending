...
public class WaterAnimSpeed : MonoBehaviour {
	...
	void Start () {
		animState = animation.clip.name; //Get the name of the animation clip.
	}
	
	void Update () {
		animation[animState].speed = speedMultiplier; //Change speed to the one set in speedMultiplier.
	}
}
